...
public class WxUser implements Serializable {
	...
	public User wxUserToUser() {
		User user = new User();
		user.setAddress(null);
		user.setAge(0);
		user.setAgentCode(null);//代理号，http请求头带有，这里不写
...
//		user.setName(this.nickname);
		...
		user.setSource(Constants.Source.WECHAT);
		user.setStatus(Constants.Status.ENABLE);
		user.setSubscribe(this.subscribe);
		user.setSubscribeTime(new Date(this.subscribe_time));
